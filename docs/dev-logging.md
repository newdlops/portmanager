...
| `agent-dns` (`portmanager_agent`) | Daemon-owned browser DNS responder: `bound udp 127.0.0.1:<port> records=…` (bind succeeded, possibly after retrying while an older extension host held the port), `sync records=… running=…` per `syncBrowserDns`, `loaded … persisted records` at startup, `recvfrom failed`/`persist failed` on socket or disk errors. |
| `agent-scan` (`portmanager_agent`) | `command deadline exceeded command=…`: async lsof/ps budget expired; the process group is terminated and partial output discarded. Registry mutations remain on the control thread; DNS persistence uses a separate publication worker. |
| `agent-publication` (`portmanager_agent`) | `routes revision=… result=…`: captured route file publication completed (`0` success). `file publication deadline exceeded`: an active file job exceeded its 8-second budget; its RPC fails and commit checks observe cancellation. |
| `agent-output` (`portmanager_agent`) | `backlog limit fd=… bytes=… frames=…` and `disconnect stalled fd=…`: bounded output queues release slow/over-budget clients without waiting inside dispatch. No response or control-command contents are logged. |
...
| `ts-browser-proxy` (extension host) | `sync routes=… desired=… active=… prepareMs=… bindMs=…` separates alias/route preparation from gateway-port reclaim and proxy binding. Compare desired and active counts when a browser alias resolves but its port is unavailable. `timeout endpoint=… phase=resolve\|connect\|queue timeoutMs=…` identifies a stalled route lookup, TCP/TLS setup, or HTTP admission wait; URLs and request headers are omitted. |
