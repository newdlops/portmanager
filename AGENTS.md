...
가진 훅된 프로세스가 **네트워크명(alphac/captainprod 등)을 호스트명으로 보게** 한다.
그 셸과 모든 자식이 상속하므로, 호스트명으로 자기를 식별하는 아무 앱이나 자동으로
per-network 분리된다(예: celery `celery@%h`→`celery@alphac`). opt-in·fail-safe
...
- 앱은 하드코딩(`-n x@localhost`) 대신 표준 호스트명 템플릿(`%h`/`%n`)을 쓰면 자동 분리 — 이건 앱 몫이지 extension 책임이 아니다.
