...
function buildShellHookStartupScript(options: ShellHookStartupScriptOptions): string {
  ...
  const startupNativeAgentProbeCommand =
    'PORT_MANAGER_HOOK_DISABLED=1 PORT_MANAGER_HOOK=0 DYLD_INSERT_LIBRARIES= LD_PRELOAD= BASH_ENV= "$PORT_MANAGER_AGENT_EXECUTABLE" --probe --socket "$PORT_MANAGER_AGENT_SOCKET" --agent-main "$PORT_MANAGER_AGENT_MAIN"';
...
# VS Code applies the small activation set after user profiles. Static routing
# context is already present at process creation, so a profile-installed hook
# must not mistake that context for an instruction to preload during startup.
if [ "\${TERM_PROGRAM:-}" = "vscode" ] && { [ -n "\${VSCODE_ENV_REPLACE:-}" ] || [ -n "\${VSCODE_ENV_PREPEND:-}" ] || [ -n "\${VSCODE_ENV_APPEND:-}" ]; }; then
...
${existingScopeRouteSelectionScript}
  ...
  unset PORT_MANAGER_TERMINAL_SESSION_ID PORT_MANAGER_TERMINAL_SESSION_NETWORK_ID PORT_MANAGER_TERMINAL_PROCESS_GROUP_ID ${EXPERIMENTAL_ROUTE_OWNERSHIP_MODE_ENV} ${ESCAPED_SERVER_RESPAWN_ENV} ${AGENT_REQUIRED_ENV}
  ${escapedShellEnvRestorePath !== undefined ? `if [ -n "\${PORT_MANAGER_PREV_BASH_ENV:-}" ] && [ "\${BASH_ENV:-}" = "${escapedShellEnvRestorePath}" ]; then export BASH_ENV="\${PORT_MANAGER_PREV_BASH_ENV}"; elif [ "\${BASH_ENV:-}" = "${escapedShellEnvRestorePath}" ]; then unset BASH_ENV; fi` : ""}
...
${options.settings.escapedServerRespawn ? `export ${ESCAPED_SERVER_RESPAWN_ENV}=1` : `unset ${ESCAPED_SERVER_RESPAWN_ENV}`}
...
if [ "\${PORT_MANAGER_HOOK:-0}" = "1" ]; then
  if [ "\${${AGENT_REQUIRED_ENV}:-${agentRequired ? "1" : "0"}}" = "1" ]; then
    # A healthy daemon is the common path. Its native probe avoids starting an
    # Electron/Node helper and keeps the large interactive command body lazy.
    if [ -x "$PORT_MANAGER_AGENT_EXECUTABLE" ] && ${startupNativeAgentProbeCommand} >/dev/null 2>&1; then
      export PORT_MANAGER_HOOK_DAEMON_STARTED=1
      export PORT_MANAGER_RUNTIME_SHIM_READY=0
      __pm_startup_shims_ready=1
      if [ ! -f "${escapedHookLibraryPath}" ]; then
        __pm_startup_shims_ready=0
      fi
      if [ -n "\${PORT_MANAGER_RUNTIME_SHIM_DIR:-}" ]; then
        ...
        if [ "$__pm_startup_shims_ready" = "1" ]; then
          for __pm_startup_shim_name in ${TERMINAL_RUNTIME_SHIM_READY_CHECK_NAMES.join(" ")}; do
            if [ ! -x "$PORT_MANAGER_RUNTIME_SHIM_DIR/$__pm_startup_shim_name" ]; then
              __pm_startup_shims_ready=0
              break
            fi
          done
...
${nativeHookPreloadScript
  ...
  .join("\n")}
        ...
        # Missing generated assets need the existing fail-safe diagnostics and
        # disable path, which remain in the lazily loaded command library.
        . "${escapedCommandLibraryPath}"
      fi
      unset __pm_startup_shims_ready __pm_startup_shim_name
    else
      # Cold/stale daemon recovery retains the serialized startup behavior.
      . "${escapedCommandLibraryPath}"
    fi
  else
${nativeHookPreloadScript
...
function buildShellHookScript(options: ShellHookScriptOptions): string {
  ...
  const nodeProbeCommand = daemonRuntimeCommand(`"${escapedNodeExecutablePath}" -e "${shellDoubleQuote(
    nodeProbeScript,
  ...
  const nodeStaleLockCommand = daemonRuntimeCommand(`"${escapedNodeExecutablePath}" -e "${shellDoubleQuote(
    staleLockScript,
  )}" "$__pm_agent_lock"`);
...
/**
 * Normalizes path-list style shell variables while adding an entry.
 * Re-sourcing profile hooks from several VS Code windows can otherwise leave
 * repeated preload or shim path segments in every child process.
 */
...
/**
 * Removes every matching entry from a path-list shell variable.
 * Detach must clean duplicate hook entries left by older generated hooks.
 */
function shellRemovePathListEntry(name: string, entry: string): string {
  const escapedEntry = shellDoubleQuote(entry);
  const removesHookLibraries = isPortManagerHookLibraryPath(entry);
  const staleHookSkip = removesHookLibraries
    ? [
        '  case "$__pm_path_entry" in',
        "    */media/native/libportmanager_hook.dylib|*/media/native/libportmanager_hook.so) continue ;;",
        "  esac",
  ...
  const removalPattern = removesHookLibraries
    ? '  *"/media/native/libportmanager_hook.dylib:"*|*"/media/native/libportmanager_hook.so:"*)'
    : `  *":${escapedEntry}:"*)`;

  /* Most VS Code profile starts have no preload entry yet. Avoid walking the
   * full PATH/DYLD list unless the exact generated entry (or an older PM hook)
   * is actually present. */
  return [
    `case ":\${${name}:-}:" in`,
    removalPattern,
    ...removalBody.map((line) => `    ${line}`),
    "    ;;",
    "esac",
  ].join("\n");
}
